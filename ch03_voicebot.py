...
def main():
    ...
    # 기본 설정
    st.set_page_config(
        page_title="이기쁨의 음성 비서 프로그램",
        layout="wide")

    # 제목
    st.header("이기쁨의 음성 비서 프로그램")
    ...
    with col2:
        ...
        if (audio.duration_seconds > 0) and (st.session_state["check_reset"]==False):
            ...
            for sender, time, message in st.session_state["chat"]:
                if sender == "user":
                    st.write(f'<div style="display:flex;align-items:center;"><div style="background-color:#007AFF;color:white;border-radius:12px;padding:8px 12px;margin-\
right:8px;">{message}</div><div style="font-size:0.8rem;color:gray;">{time}</div></div>', unsafe_allow_html=True)
                    st.write("")
                else:
                    st.write(f'<div style="display:flex;align-items:center;justify-content:flex-end;"><div style="background-color:lightgray;border-radius:12px;padding:8px \
12px;margin-left:8px;">{message}</div><div style="font-size:0.8rem;color:gray;">{time}</div></div>', unsafe_allow_html=True)
                    st.write("")
